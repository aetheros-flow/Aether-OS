...
export default function PersonalGrowthPage() {
  ...
  return (
    <div className="min-h-screen bg-[#FAF9F6] text-[#2D2A26] font-sans selection:bg-purple-200">

      {/* BACKGROUND GLOWS */}
      <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
        <div className="absolute top-[-10%] right-[-10%] w-[600px] h-[600px] bg-purple-100/50 rounded-full blur-[120px]" />
        <div className="absolute bottom-[-10%] left-[-10%] w-[500px] h-[500px] bg-blue-100/30 rounded-full blur-[100px]" />
      </div>
      ...
      <div className="max-w-6xl mx-auto px-6 pt-10 relative z-10">
        ...
        <div className="flex justify-between items-center mb-12">
          ...
          <div className="flex bg-white/60 backdrop-blur-xl p-1 rounded-full border border-black/5 shadow-inner">
            {TABS.map(tab => (
              <button
                key={tab.id}
                onClick={() => setActiveTab(tab.id)}
                className={`flex items-center gap-2 px-6 py-2 rounded-full text-[10px] font-extrabold uppercase tracking-widest transition-all ${activeTab === tab.id ? 'bg-[#2D2A26] text-white shadow-lg' : 'text-[#8A8681] hover:text-[#2D2A26]'}`}
              >
        ...
        {/* HEADER */}
        <div className="mb-16">
          <h1 className="aether-title text-5xl md:text-6xl mb-4">Evolution</h1>
          <p className="text-[11px] font-bold text-[#8A8681] uppercase tracking-[0.3em]">Module: Human Potential / Leveling UP</p>
        </div>
            ...
            {skills.map(skill => (
              <div key={skill.id} className="bg-white/80 backdrop-blur-xl rounded-[32px] p-8 border border-black/5 shadow-sm hover:shadow-xl transition-all group">
                ...
                <div className="mt-6 pt-6 border-t border-black/5 flex justify-between items-center">
                  <span className="text-[10px] font-bold text-[#8A8681]">{skill.hours_invested ?? 0} Hours Practice</span>
                  <ChevronRight size={14} className="text-[#8A8681] opacity-0 group-hover:opacity-100 transition-opacity" />
                </div>
              </div>
            ))}
          </div>
        ...
        {activeTab === 'ikigai' && (
          <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="bg-[#2D2A26] rounded-[32px] p-10 text-white flex flex-col md:flex-row justify-between items-center gap-8 shadow-2xl">
            ...
            <div className="grid grid-cols-1 gap-4">
              {ikigaiLogs.map(log => {
                ...
                return (
                  <div key={log.id} className="bg-white/70 backdrop-blur-md border border-black/5 p-6 rounded-[32px] flex items-center justify-between group hover:bg-white transition-all shadow-sm">
