...
// 2. Función matemática para dibujar los gajos polares
...
export default function App() {
  const [data, setData] = useState(universes);
  const [hoveredUniverse, setHoveredUniverse] = useState<string | null>(null);

  // Configuración del SVG y Gráfico (Lienzo GIGANTE para que al reducirse en mobile se lea bien)
  const size = 1200; 
  const center = size / 2;
  const maxRadius = 260; 
  const anglePerSlice = 360 / data.length;

  // Actualizador de puntajes
  const updateScore = (id: string, newScore: number) => {
    setData(data.map(item => item.id === id ? { ...item, score: newScore } : item));
  };

  // Lógica de Tinte Dinámico
  const activeUniverseData = data.find(u => u.id === hoveredUniverse);
          ...
          <div className="w-full h-full relative z-10">
            <svg width="100%" height="100%" viewBox={`0 0 ${size} ${size}`} className="overflow-visible">
              ...
              {data.map((item, i) => {
                ...
                // Aumentamos el radio muchísimo para dar espacio a los textos grandes
                ...
                return (
                  ...
                  >
                    ...
                    {/* Líneas divisorias (Más gruesas para el lienzo grande) */}
                    <line 
                      x1={center} y1={center} 
                    ...
                    {/* DISEÑO ORBITAL: Tamaños Masivos para asegurar lectura en mobile */}
                    <foreignObject 
                      x={labelPos.x - 170} 
                      y={labelPos.y - 80} 
                      width="340" 
                    ...
                    >
                      <div className={`flex flex-col items-center justify-center h-full transition-all duration-500 ${isAnotherHovered ? 'opacity-50' : 'opacity-100'}`}>
                        ...
                        {/* Texto gigante en el SVG (32px) para que al reducirse en el celu quede perfecto */}
                        <span 
