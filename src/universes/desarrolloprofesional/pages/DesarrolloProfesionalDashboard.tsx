...
import { ArrowLeft, LayoutDashboard, Briefcase, Menu, X } from 'lucide-react';
       ...
       <nav className="w-full md:w-64 flex flex-col z-30 shrink-0 transition-all duration-300 border-r border-white/10" style={{ backgroundColor: theme.bg }}>
           <div className="flex items-center justify-between p-4 md:p-6 md:mb-6">
                <div className="flex items-center gap-4">
                  <button onClick={() => navigate('/')} className="p-2 rounded-full hover:bg-black/10 transition-colors text-black"><ArrowLeft size={20} /></button>
                  <div>
                    <h1 className="aether-title" style={{ color: theme.textMain }}>Profesional</h1>
                    <p className="aether-eyebrow" style={{ color: 'rgba(0,0,0,0.7)' }}>Carrera & Propósito</p>
                  </div>
                </div>
                <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="md:hidden p-2 rounded-xl hover:bg-black/10 transition-colors" style={{ color: 'rgba(0,0,0,0.7)' }}>
       ...
       <main className="flex-1 p-4 md:p-10 overflow-y-auto pb-32 md:pb-10">
          <header className="mb-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
             <div>
                ...
                <div className="aether-metric-xl flex items-center gap-3" style={{ color: theme.textMain }}>
                   En Calibración <Briefcase strokeWidth={3} className="text-white opacity-80" />
                </div>
             </div>
          </header>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div className="aether-card h-64 flex flex-col justify-center items-center text-center col-span-full" style={{ backgroundColor: theme.surface }}>
               ...
               <p className="aether-eyebrow opacity-70" style={{ color: theme.textMuted }}>Próximamente desplegaremos la matriz de datos para tu Desarrollo Profesional.</p>
            </div>
          </div>
       </main>
    </div>
